...
public class DateTimeAndDurationProviderExample {
  ...
  public void print() {
    ...
    this.out.println(
        "Date between 2 timestamps : "
            + dateAndTime.between(Timestamp.from(Instant.EPOCH), Timestamp.from(Instant.now())));
    this.out.println(
        "Date between 2 timestamps : "
            + dateAndTime.between(
                Timestamp.from(Instant.EPOCH), Timestamp.from(Instant.now()), "dd/MM/yyyy"));

    this.out.println(
        "A date between 10 and 50 days in the future : "
            + dateAndTime.future(50, 10, TimeUnit.DAYS, "dd/MM/yyyy"));
    this.out.println(
        "A date between 10 and 50 days in the past : "
            + dateAndTime.past(50, 10, TimeUnit.DAYS, "dd/MM/yyyy"));

    this.out.println(
        "A period between an lower and an upper period : "
            + dateAndTime.period(Period.ofYears(5), Period.ofYears(10)));
  }
}
